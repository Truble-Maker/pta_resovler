...
class PTAContestGenerator:
    ...
    def get_problem_sets(self):
        ...
        return [{
            ...
            "problem_count": ps.get("problemCount", 0)
        } for ps in all_sets]
    ...
    def _process_exam_info(self):
        """处理考试基础信息"""
        url = f'https://pintia.cn/api/problem-sets/{self.selected_problem_set_id}/exams'
        resp = self.session.get(url)
        ...
        ET.SubElement(info, "penalty").text = str(self.exam_info.get("penalty_minutes", 20))
        ET.SubElement(info, "started").text = "False"
        ET.SubElement(info, "starttime").text = f"{start_time:.1f}"
        ET.SubElement(info, "title").text = self.exam_info.get("title", "默认比赛")
        ET.SubElement(info, "short-title").text = self.exam_info.get("short_title", "Default Contest")
        ET.SubElement(info, "scoreboard-freeze-length").text = self.exam_info.get("freeze_duration", "1:00:00")
